...
public class GlowSquidRenderer extends MobRenderer<GlowSquidEntity.CustomEntity, SquidModel<GlowSquidEntity.CustomEntity>> {
    ...
    public void render(GlowSquidEntity.CustomEntity entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
        super.render(entityIn, entityYaw, partialTicks, matrixStackIn, bufferIn, packedLightIn);
        this.currentFrame = (int) (Math.floor(entityIn.ticksExisted / ANIMATION_TIME) % ANIMATION_FRAMES);
    }

    public ResourceLocation getEntityTexture(GlowSquidEntity.CustomEntity entity) {
//        String frameLocation = MessageFormat.format("earthtojavamobs:textures/mobs/squid/glow_squid/glow_squid_anim_{0}.png", (currentFrame + 1));
    ...
    protected void applyRotations(GlowSquidEntity.CustomEntity entityLiving, MatrixStack matrixStackIn, float ageInTicks, float rotationYaw, float partialTicks) {
    ...
    protected float handleRotationFloat(GlowSquidEntity.CustomEntity livingBase, float partialTicks) {
