...
public class LobberZombieModel<T extends LobberZombieEntity> extends ZombieEntityModel<T> {
	...
	public static ModelData getModelData(Dilation dilation, float pivotOffsetY) {
		...
		modelPartData.addChild(EntityModelPartNames.RIGHT_ARM, ModelPartBuilder.create().uv(16, 32).cuboid(-3.0F, -2.0F, -2.0F, 4.0F, 11.0F, 4.0F, dilation), ModelTransform.pivot(-5.0F, 2.0F + pivotOffsetY, 0.0F));
		modelPartData.addChild(EntityModelPartNames.LEFT_ARM, ModelPartBuilder.create().uv(32, 32).mirrored().cuboid(-1.0F, -2.0F, -2.0F, 5.0F, 14.0F, 4.0F, dilation), ModelTransform.pivot(5.0F, 2.0F + pivotOffsetY, 0.0F));
