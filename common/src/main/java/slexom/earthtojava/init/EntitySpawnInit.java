...
public final class EntitySpawnInit {
    ...
    private static <T extends AnimalEntity> void registerAnimalEntitySpawn(RegistrySupplier<EntityType<T>> entity, ModConfig.EntityConfig entityConfig) {
        if (entityConfig.spawn) {
            BiomeSpawnHelper.setCreatureSpawnBiomes(entity, entityConfig.spawnBiomes.toArray(new String[0]), entityConfig.weight, entityConfig.groupMin, entityConfig.groupMax);
        }
    }

    private static <T extends HostileEntity> void registerMonsterEntitySpawn(RegistrySupplier<EntityType<T>> entity, ModConfig.EntityConfig entityConfig) {
        if (entityConfig.spawn) {
            BiomeSpawnHelper.setMonsterSpawnBiomes(entity, entityConfig.spawnBiomes.toArray(new String[0]), entityConfig.weight, entityConfig.groupMin, entityConfig.groupMax);
        }
    }
